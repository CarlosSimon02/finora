...
export const addMoneyToPot = (potRepository: IPotRepository) => {
  const useCase = async (
    ...
    // Business rule: Cannot exceed maximum total
    if (data.amount + pot.totalSaved > COMMON_MAX_NUMBER) {
      throw new DomainValidationError("Amount exceeds maximum limit");
    }

    // Domain validation passed, delegate to repository
    return potRepository.addToTotalSaved(userId, potId, data.amount);
  };
